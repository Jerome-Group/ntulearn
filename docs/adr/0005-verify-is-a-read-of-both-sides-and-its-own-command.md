...
- **The path it predicts is the path a sync writes**, because both ask `src/sync/placement.mjs`
  rather than each walking the content tree its own way. A change to naming that goes through one
  and not the other is the failure this arrangement exists to make impossible.
